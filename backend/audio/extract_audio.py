    ...
    if local_appdata:
        ...
        if winget_root.exists():
            for package_dir in winget_root.glob("Gyan.FFmpeg*"):
                candidates.extend(str(path) for path in package_dir.rglob("ffmpeg.exe"))
...
def extract_audio(video_path: str, output_path: str) -> str:
    ...
    try:
        ...
        raise RuntimeError(
            "FFmpeg executable could not be started. Verify FFmpeg is installed correctly and available on PATH."
        ) from e
